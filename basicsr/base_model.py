...
class BaseModel():
    ...
    def save_network(self, net, net_label, current_iter, param_key='params'):
        ...
        for net_, param_key_ in zip(net, param_key):
            ...
            for key, param in state_dict.items():
                ...
                # if 'daclip_model' not in key and 'l0_experts' not in key and 'l1_experts' not in key and 'l2_experts' not in key:
    ...
    @master_only
    def save_accelerator_network(self, net, net_label, current_iter, param_key='params'):
        """Save networks.
        ...
        save_dict = {}
        for net_, param_key_ in zip(net, param_key):
            net_ = self.accelerator.unwrap_model(net_)
            net_ = self.get_bare_model(net_)
            state_dict = net_.state_dict()
            for key, param in state_dict.items():
    ...
    @master_only
    def save_lora_network(self, net, net_label, current_iter, param_key='params'):
        """Save networks.
        ...
        for net_, param_key_ in zip(net, param_key):
            net_ = self.get_bare_model(net_)
            # state_dict = net_.state_dict()
            state_dict = lora.lora_state_dict(net_)
            for key, param in state_dict.items():
                if key.startswith('module.'):  # remove unnecessary 'module.'
                    key = key[7:]
                state_dict[key] = param.cpu()
    ...
    @master_only
    def save_lora_network_w_norm_temp(self, net, net_label, current_iter, param_key='params'):
        """Save networks.
        ...
        for net_, param_key_ in zip(net, param_key):
            ...
            state_dict = net_.state_dict()
            lora_state_dict = lora.lora_state_dict(net_)

            for key, param in lora_state_dict.items():
                if key.startswith('module.'):  # remove unnecessary 'module.'
                    key = key[7:]
                lora_state_dict[key] = param.cpu()

            for key, param in state_dict.items():
                if key.startswith('module.'):  # remove unnecessary 'module.'
                    key = key[7:]
                if "norm" in key or "temperature" in key:
                    lora_state_dict[key] = param.cpu()

            save_dict[param_key_] = lora_state_dict

        torch.save(save_dict, save_path)

    @master_only
    def save_router_network(self, net, net_label, current_iter, param_key='params'):
        """Save networks.
        ...
        for net_, param_key_ in zip(net, param_key):
            net_ = self.get_bare_model(net_)
            state_dict = net_.state_dict()
            to_save_dict = OrderedDict()
            for key, param in state_dict.items():
                if key.startswith('module.'):  # remove unnecessary 'module.'
                    key = key[7:]
                if 'expert' not in key and 'daclip_model' not in key:
                    to_save_dict[key] = param.cpu()

            save_dict[param_key_] = to_save_dict

...
def img2tensor_vae(imgs, bgr2rgb=True, float32=True):
    def _totensor(img, bgr2rgb, float32):
        if img.shape[2] == 3 and bgr2rgb:
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        img = torch.from_numpy(img.transpose(2, 0, 1))
...
def padding_tri(img_lq, img_gt, img_ref, gt_size):
    h, w, _ = img_lq.shape
    ...
    if h_pad == 0 and w_pad == 0:
        return img_lq, img_gt, img_ref

    img_lq = cv2.copyMakeBorder(img_lq, 0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)
    img_gt = cv2.copyMakeBorder(img_gt, 0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)
    img_ref = cv2.copyMakeBorder(img_ref, 0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)
...
def padding_single_img(img_gt, gt_size):
    h, w, _ = img_gt.shape

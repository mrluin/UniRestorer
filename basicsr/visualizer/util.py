...
def augment_funcHWC(img, hflip, vflip, rot90):  # HxWxC
    if hflip:   img = img[:, ::-1, :]
    if vflip:   img = img[::-1, :, :]
...
def augmentHWC(*imgs, hflip=True, vflip=True, rot=True):  # CxHxW
    if hflip == True:
        hflip = random.random() < 0.5
    else:
        hflip == False
    ...
    if rot == True:
        ...
        rot90 = False
    return list(augment_funcHWC(img, hflip, vflip, rot90) for img in imgs)
